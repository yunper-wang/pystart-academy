    ...
    for chapter in get_chapters(data):
        for index, exercise in enumerate(chapter.get('exercises', [])):
            ...
            item.update({
                ...
                'index': index,
                'displayTitle': exercise.get('title') or exercise.get('text') or f"练习 {index + 1}",
                'taskGoal': exercise.get('taskGoal') or exercise.get('description') or exercise.get('text') or '',
                'starter': starter,
