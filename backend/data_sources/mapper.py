...
def map_to_exercise(raw: dict, counter: int = 0) -> dict:
    ...
    exercise = {
        ...
        'starter': (raw.get('starter') or '').strip()[:5000],
        'expectedOutput': (raw.get('expectedOutput') or '').strip()[:1000],
        'answer': (raw.get('answer') or '').strip()[:1000],
        'answerCode': (raw.get('answerCode') or raw.get('content') or '').strip()[:10000],
