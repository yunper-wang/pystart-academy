...
专为编程零基础或刚入门的学习者设计，提供 30 个循序渐进的章节、240 道 curated v2 原创优质练习题、8 个项目实战，以及内置的 Python 运行环境，让你在浏览器里就能写代码、跑代码、学编程。
...
新版题库已统一重建为 curated v2：每章 8 道原创题，总计 240 道。每章难度分布固定为 3 道基础、3 道进阶、2 道挑战，所有题目都包含可运行参考答案、预期输出、解析、标签、测试样例和质量说明。
...
| 第6章 | 运算符 | 8 |
| 第7章 | 条件判断 | 8 |
| 第8章 | for 循环和 while 循环 | 8 |
| 第9章 | break 和 continue | 8 |
| 第10章 | 字符串常用操作 | 8 |
...
| 第26章 | 文本文件小脚本实践 | 8 |
| 第27章 | 异常处理与健壮程序 | 8 |
| 第28章 | 日期、时间与简单自动化脚本 | 8 |
| 第29章 | 综合脚本项目：个人学习记录器 | 8 |
| 第30章 | 综合复盘与小型脚本开发流程 | 8 |

### 题库质量治理

当前版本已经清理旧题库：

- 删除早期 `legacy/unknown` 历史题
- 删除 `futurecoder-authorized-copy` 导入题
- 删除上一版零散增强题 `pystart-quality-original-v1`
- 统一重建为 `pystart-curated-v2` 原创题库

新版题库标准：

- 30 章 × 8 题 = 240 道
- 每章固定 3 道基础、3 道进阶、2 道挑战
- 每题都有稳定 `id`、`title`、`description`、`direction`、`tags`
- 每题都有 `starter` 起始代码、`answerCode` 参考答案、`expectedOutput` 预期输出
- 每题都有 `analysis` 解题思路、`examples` 示例、`tests` 校验样例、`qualityNotes` 质量说明
- 所有参考答案都由校验脚本真实执行并比对输出

题库维护命令：

```bash
# 重新生成 curated v2 题库，会清空旧 exercises 并写入 240 道新题
python3 scripts/build_curated_question_bank.py

# 校验题量、来源、难度分布、字段完整性和全部参考答案输出
python3 scripts/validate_curated_question_bank.py
```

首次重建时会保留备份文件：`data.backup-before-curated-v2.json`。
...
新增题库配套了校验脚本，检查新增题是否包含标题、描述、难度、标签、示例、参考答案、解析、测试用例和覆盖方向：
...
```bash
python3 scripts/add_quality_exercises.py
```
